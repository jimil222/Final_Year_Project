...
from app.core.dependencies import get_current_student, get_current_admin, get_current_user
...
async def request_book(book_id: int, current_user=Depends(get_current_student)):
  ...
  if book.status != "AVAILABLE":
    raise HTTPException(
      status_code=400, detail=f"Book is not available (current status: {book.status})"
    )
