    ...
    <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 bg-white rounded-lg p-1 border border-gray-200 shadow-sm">
        <button
          ...
          type="button"
          className={`flex-1 py-3 rounded-md font-semibold text-sm transition-all duration-200 ${mode === 'issue'
        ...
        <button
          ...
          type="button"
          className={`flex-1 py-3 rounded-md font-semibold text-sm transition-all duration-200 ${mode === 'return'
              ? 'bg-blue-600 text-white shadow-sm'
              : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
            }`}
        >
      ...
      {result && (
        <div
          className={`card ${result.success ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'
            }`}
        >
          <div className="flex items-start space-x-4">
            <div
              className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 ${result.success ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
                }`}
            >
              {result.success ? <Check className="text-xl" /> : <X className="text-xl" />}
            </div>
            <div className="flex-1">
              <h3
                className={`text-lg font-bold mb-2 ${result.success ? 'text-green-900' : 'text-red-900'
                  }`}
              >
